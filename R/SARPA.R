#' SARPA method in GRAB package
#' 
#' SARPA method is an empirical approach to analyzing complex traits (including but not limited to longitudinal trait) for related samples in a large-scale biobank.
#' 
#' @details 
...
#' obj.SARPA = SARPA.NullModel(ResidMatFile = ResidMatFile,
...
#' @export
GRAB.SARPA = function(){
  print("Check ?GRAB.SARPA for more details about 'SARPA' method.")
}

################### This file includes the following functions

# ------------- This file includes the following functions
# 1. checkControl.Marker.SARPA(control)
# 2. setMarker.SARPA(objNull, control)
# 3. mainMarker.SARPA()

# check the control list in marker-level testing
checkControl.Marker.SARPA = function(control)
{
...
checkControl.SARPA.NullModel = function(control,
                                        Pheno,
                                        ResidMat,
                                        SparseGRM,
                                        PairwiseIBD)
{
  ...
  if(any(!SubjID.In.Pheno %in% SubjID.In.Pheno))
    stop("At least one subject in residual matrix does not have phenotype information.")
...
mainMarker.SARPA = function(genoType, genoIndex, outputColumns)
{
...
SARPA.NullModel = function(PhenoFile,          # at least four columns: column 1 is subjID, column 2 is Target phenotype, column 3 is Risk prediction, column 4 covariates
                                          ...
                                          CovarColList = NULL))
{
  ...
  control = checkControl.SARPA.NullModel(control, Pheno, ResidMat, SparseGRM, PairwiseIBD)
  ...
  class(obj) = 'SARPA_NULL_Model'
  return(obj)
}

SARPA_fit_null = function(tarVec,
                          riskVec,
                          covMat){
  
  # Stage 2 probit regression.
  fit = glm(tarVec ~ covMat + riskVec, family = binomial(link = "probit"), na.action = na.omit)
  
  coef = summary(fit)$coefficients
  
  gammas = coef[rownames(coef) != 'riskVec', 'Estimate']
  gamma.riskVec = coef['riskVec', 'Estimate']
  
  names(gammas) = paste("gamma", names(gammas), sep = '.')
  names(gamma.riskVec) = 'gamma.riskVec'
  
  out = list('gammas' = gammas, 'gamma.riskVec' = gamma.riskVec)
  return(out)
}

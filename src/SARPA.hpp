...
private:
  ...
  Rcpp::List m_gammas;                   // results of fit null logistic regression (tarVec ~ designMat + riskVec)
...
public:
  ...
  arma::vec cal_resid(arma::vec t_Gvec,
                      Rcpp::List t_fit)
  {
    ...
    arma::vec t = arma::join_horiz(X, new_riskvec) * combined_gammas;
    ...
    double ia2b2 = - gamma_riskVec * arma::sum(arma::pow(f, 2) % arma::pow(t_Gvec.elem(obs_Y), 2) / deno);
    double ib2b2 = arma::as_scalar(arma::sum(arma::pow(t_Gvec, 2)) / sigma2_sq) - (gamma_riskVec * ia2b2);
    ...
    arma::vec R2_beta2 = - gamma_riskVec * f % (m_Tarvec.elem(obs_Y) - F) / deno;
